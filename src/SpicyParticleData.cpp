...
void godot::ParticleData::delete_data()
{
	initialized = false;
...
godot::ParticleData::~ParticleData()
{
	if (!initialized)
		return;
	delete_data();
...
void godot::ParticleData::generate(size_t max_size, const Ref<RandomNumberGenerator>& p_rng)
{
...
void godot::ParticleData::kill(size_t id)
{
	if (count_alive > 0)
	{
		alive[id] = false;
...
void godot::ParticleData::wake(size_t id)
{
...
void godot::ParticleData::swap(size_t id1, size_t id2)
{
...
void godot::ParticleData::get_transform(size_t id, Transform3D& p_transform) const
{
...
void godot::ParticleData::get_color(size_t id, Color& p_color) const
{
	p_color = color[id] * current_color[id];
}

size_t godot::ParticleData::get_last_alive_index()
{
