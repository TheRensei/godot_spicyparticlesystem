...
////////////////////////////////////////////////////////////////////////////////
// ParticleBurst
void godot::SpicyParticleBurst::_bind_methods()
{
...
void godot::SpicyParticleEmitter::_bind_methods()
{
	ClassDB::bind_method(D_METHOD("sort_bursts", "a", "b"), &SpicyParticleEmitter::sort_bursts);
}

void godot::SpicyParticleEmitter::emit(double dt, const Ref<ParticleData> p_data)
{
	int to_emit = 0;
...
void godot::SpicyParticleEmitter::emit_burst(double dt, const Ref<ParticleData> p_data, int count)
{
...
void godot::SpicyParticleEmitter::reset()
{
...
void godot::SpicyParticleEmitter::add_generator(const Ref<SpicyParticleGenerator>& generator)
{
	m_generators.push_back(generator);
}

void godot::SpicyParticleEmitter::remove_generator(const Ref<SpicyParticleGenerator>& generator)
{
	m_generators.erase(generator);
}

void godot::SpicyParticleEmitter::set_bursts(const TypedArray<SpicyParticleBurst>& bursts)
{
	m_bursts = bursts;
...
void godot::SpicyParticleEmitter::reset_bursts()
{
...
bool godot::SpicyParticleEmitter::sort_bursts(const Ref<SpicyParticleBurst>& a, const Ref<SpicyParticleBurst>& b)
{
...
godot::SpicyParticleSystem::SpicyParticleSystem() : is_emitting(false), count(0)
{
	particles = Ref<ParticleData>(memnew(ParticleData));
}

godot::SpicyParticleSystem::~SpicyParticleSystem()
{

}

void godot::SpicyParticleSystem::initialize(size_t max_count, const Ref<RandomNumberGenerator>& p_rng, const Node3D& particle_node)
{
	count = max_count;
...
void godot::SpicyParticleSystem::set_max_particle_count(size_t max_count, const Ref<RandomNumberGenerator>& p_rng)
{
	count = max_count;
	particles->generate(max_count, p_rng);
}

void godot::SpicyParticleSystem::delete_particle_data()
{
	particles->delete_data();
}

void godot::SpicyParticleSystem::update(double dt, double current_duration_normalized, const Transform3D& node_transform)
{
...
void godot::SpicyParticleSystem::reset()
{
	is_emitting = false;
...
void godot::SpicyParticleSystem::emit(double dt, int count)
{
	is_emitting = true;
...
void godot::SpicyParticleSystem::set_emitting(bool p_emitting)
{
